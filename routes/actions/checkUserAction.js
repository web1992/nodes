...
router.get("/checkUserInfo", function(req, res, next) {
  ...
  var channel_code = "";
  var reqParam =
    "?token=" + token + "&uid=" + uid + "&product_code=" + productCode;
  console.log("checkUserAction#reqParam=" + reqParam);
  axios
    .get(checkUserUrl + reqParam)
    .then(function(response) {
      const data = response.data;
      console.log("checkUserAction#data=" + data);
      if (SUC == data) {
        res.send(
          JSON.stringify({ code: 200, msg: "验证用户信息成功", data: true })
        );
      } else {
        res.send(
          JSON.stringify({ code: 200, msg: "验证用户信息失败", data: false })
        );
      }
    })
    .catch(function(error) {
      ...
      console.log(error);
      res.send(
        JSON.stringify({ code: 200, msg: "验证用户信息异常", data: false })
      );
    })
    .then(function() {
      // always executed
